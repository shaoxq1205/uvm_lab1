...
/**
 * @brief QIRQ PROXY DPI functions
 *
 * The QIRQ PROXY DPI functions c++ implementation. 
 *
 * @file qirq_proxy_dpi.cpp
 *
 ...
 *
 * $Revision: 1.2 $
 * $Date: Tue Mar  8 12:54:53 2011 $
 * $Author: mironm $
 */

#ifndef QIRQ_PROXY_DPI__CPP
#define QIRQ_PROXY_DPI__CPP
...
//------------------------------------------------
  ...
  /**
   * @brief qirq_dpii_initi_signal_table
   ...
   * @param num_items    (int) - number of elements expected
   * @param sigtable[]  (char *) - array containaing the returned items
   */


void qirq_dpii_init_signal_table( const char *pin_map_file, 
				  int *num_items, 
#ifndef PALLADIUM
				  char * sigtable[]
#else
...
)
{
  ...
  stringstream ss;               ///  stringstream used for parsing
  ...
  while (std::getline(ifile, line_read))
  {
    ...
    if(tokens.size() != 4) { //probably invalid line
      cout << "Error: Invalid line in pinmap file (Line no :  " << curr_line <<  "; Tokensize : "  << tokens.size() 
	   << ");" <<endl;

      for(int i = 0; i < (int) tokens.size(); i++) {
	cout << i << " = " << tokens[0].c_str() << endl;
      }
      return;
    }
    ...
    char *new_str_ptr = (char *) malloc(tokens[2].size());
